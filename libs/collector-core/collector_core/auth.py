"""Bearer-token authentication, shared by every collector API.
...
def _rejection(
    request: Request, exempt_paths: frozenset[str]
) -> tuple[int, str] | None:
...
def build_bearer_middleware(metrics: CollectorMetrics, exempt_paths: frozenset[str]):
    """Build the ASGI `http` middleware callable for one collector.

    `metrics` carries the collector's own label, so an auth rejection lands
    under the right `collector` series without this module knowing which
    collector it is running in.
    """

    async def require_bearer_token(request: Request, call_next):
        rejection = _rejection(request, exempt_paths)
        if rejection is None:
            return await call_next(request)

        status, reason = rejection
        metrics.auth_failure(reason)
