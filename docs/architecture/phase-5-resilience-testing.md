...
> **Status:** ✅ **Done** — delivered in #45, #46, #47, #48 · [roadmap](../../README.md#phases)

**Goal:** Prove the platform survives real-world conditions — not just happy-path deployments. Introduce rigorous service and platform-level testing in two escalating stages: exhaustive automated testing, then chaos and scale validation.
...
The two stages build on each other:
...
**Why two escalating stages, not one combined phase.**
Each stage validates a different threat model: Stage 1 catches developer mistakes before they ship. Stage 2 catches infrastructure failures that CI cannot simulate. Combining them would make failures harder to attribute and the scope too large to execute incrementally.

**Why chaos criteria are Prometheus queries, not tool assertions.**
Chaos Mesh injects but cannot judge — its `StatusCheck` supports `statusCode` only, and Prometheus answers `200` even for an empty result. A scenario's hypothesis is therefore evaluated by `scripts/run-chaos.py` against real series, and an empty result is a hard error rather than a zero. A criterion that no observation could falsify is rejected outright: this phase shipped three checks that could not fail, and every one was caught by running it rather than reading it.

**Why the load harness ships without its regression gate.**
The >20% P95 gate was specified, but `player-projections` serves an empty list in stub mode, so a ramp measures uvicorn's overhead rather than the service's. A gate built on those numbers fires on noise until somebody disables it, at which point it is worse than absent. The harness, scripts, and CI wiring exist now; the gate waits for real documents. See [`scale-baselines.md`](../scale-baselines.md).

---

## Definition of Done

- [x] All stage deliverables implemented and merged to `main`
- [x] Tests green in CI; integration gate passing
- [x] This doc's Status banner flipped to ✅ **Done** with the delivering PR
- [x] README Phases table updated (Status + Landed)
- [x] Milestone commit tagged `phase-5` and pushed — see [tagging-policy.md](../tagging-policy.md)
