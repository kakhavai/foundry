...
Current services are `weather` (current conditions by location, with a pro football stadium endpoint planned for game-day forecasts) and `player-projections` (polls an internal player-data backend; stubs gracefully until that upstream is built).
