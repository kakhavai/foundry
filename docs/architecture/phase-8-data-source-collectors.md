...
| `external_ids` | object | `{ <source>: { id, linked_at, link_method } }` where `link_method` is `exact_id`, `exact_name_team`, or `fuzzy` |
...
`name_resolution_miss` rows carry `raw_name`, `source`, `context` (team/position hints supplied by the caller), `best_candidate_player_id`, `confidence`, `first_seen_at`, `occurrence_count`.
...
**Adapter notes:** An adapter must supply, at minimum, a stable upstream id, a name, a team, and a position; anything less cannot be linked and must be routed to the miss queue rather than guessed. Linking is tiered and the tiers must not be collapsed: exact upstream-id match first, then exact `(normalized_key, team, position_group)`, then fuzzy scoring, and each tier records its own `link_method` so a downstream consumer can refuse fuzzy links if it wants to. The genuinely hard part is not the string distance function — it is deciding when two records that look alike are the *same* player versus when one player's record should *split* because an id was reused, and that decision needs birth date and entry year, which several upstreams do not publish.

**Failure mode to watch:** A high-confidence wrong merge. Two active players share a name (a WR and a safety both named Michael Thomas; a rookie entering the league under the normalized key of a recently retired veteran), the fuzzy resolver returns `confidence: 1.0` on exact string equality, and every downstream signal for both players silently lands on one `player_id` — producing a plausible, fully-populated, entirely wrong usage profile. Confidence-threshold alerting cannot catch this because the confidence is genuinely high. The assertion that does: within a season, `(normalized_key, position_group)` must resolve to exactly one `player_id`, and any collision must be forced to disambiguate on `birth_date` or `entry_year` before linking, incrementing `identity_ambiguous_name_total`; pair it with an invariant that each `external_ids.<source>.id` is injective per season, alerting on `identity_merge_conflicts_total > 0`. A cheap cross-check catches the rest: a `player_id` whose `position_group` is `offense_skill` accumulating defensive snap counts is a merge, not a trick play. Separately, assert `team_as_of` is within the requested `scope.week` window — a stale team field yields signals attributed to the wrong offense with no other symptom.
...
| `resolution_method` | enum | `exact`, `crosswalk`, `fuzzy`, `unresolved` |
...
| `entities[].resolution_confidence` | float | 0.0–1.0 from `player-identity` fuzzy resolution |
...
**Adapter notes:** An adapter normalizes one source family into the item shape: fetch, extract publication time and body text, and emit unlinked items — entity extraction, classification, `assertion_type`, and clustering are collector-side so that every source is judged by the same rules. Entity linking goes through `player-identity`'s fuzzy resolution and must record `resolution_confidence` rather than dropping low-confidence links, because a link at 0.4 confidence is information the consumer can threshold and a silently dropped link is not. Clustering is the hard part: the same story arrives with different headlines minutes apart, and near-duplicate detection has to be tight enough to collapse them while still separating a genuine follow-up report (which becomes `supersedes_item_id`) from a restatement.
