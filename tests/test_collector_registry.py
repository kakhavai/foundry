...
def test_entry_is_deployed_by_gitops(entry):
    """A registered collector must actually be deployed.

    There is no longer an `infra/gitops/argo/<name>.yaml` to look for: the
    Applications are generated by `infra/gitops/argo/applicationset.yaml`,
    whose git directory generator globs `helm/values/*`. So the two things
    that must exist per collector are the values directory the generator
    discovers it by, and the image-tag file the generated Application
    references as its second valueFile — a missing one of those renders as a
    FAILED Application rather than an absent one.

    That the generator's pattern really does match, and that a brand-new
    collector is covered without any per-service file, is
    tests/test_service_ci_coverage.py.
    """
    name = entry["name"]
    for relative in (
        Path("helm") / "values" / name / "values.yaml",
...
def test_entry_envelope_version_matches_collector_core(entry):
    ...
    Both sides are strings now — see contracts/collector-registry.yaml's
    header. The previous str()-on-both-sides comparison was a workaround for
    an undecided type rather than a decision, and it would also have passed
    `1` against `"1.0"`.
    """
    assert entry["envelope_version"] == ENVELOPE_VERSION, (
...
def test_compare_rejects_an_int_envelope_version():
    """The int/string split is settled: both sides are the string `"1"`.

    This test used to assert the opposite — that `1` and `"1"` agreed — while
    the type was undecided. The decision was to make the registry a string
    (everything else in the repo already was: the envelope schema's
    `{"const": "1"}`, every fixture, ENVELOPE_VERSION, and the lake path
    segment `v1`), so the coercion is gone and a drift back to an integer is
    caught rather than absorbed.
    """
    entry = dict(ENTRIES[0], envelope_version=1)
    catalog = dict(_catalog_for(entry), envelope_version="1")
    problems = check_registry.compare_entry_to_catalog(entry, catalog)
    assert any("envelope_version drift" in problem for problem in problems), problems
