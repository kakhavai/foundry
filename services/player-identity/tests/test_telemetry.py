"""player-identity's telemetry wiring.
...
file asserts only what is genuinely this service's: that *this* app resolves
the shared module under the env guard, and names itself `player-identity`.
"""
...
from player_identity.capture import COLLECTOR_NAME
from player_identity.main import app


def test_player_identity_uses_the_shared_telemetry_module():
...
def test_setup_is_called_with_this_collectors_own_name(monkeypatch):
    ...
    assert resolve_service_name(calls[0]) == "player-identity"
