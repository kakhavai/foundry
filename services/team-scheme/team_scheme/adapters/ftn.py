...
parquet variant is 0.53 MiB; see `docs/collectors.md` for why that saving did
not buy `pyarrow`. Measured through the shipped path: 0.5s for 47,316 rows.
...
`capture` never reaches it when play-by-play failed, because the index it
needs is exactly what a missing play-by-play denies it.
"""
...
async def fetch_charting_buckets(
    ...
    *,
    ...
    Raises on an upstream failure. `capture` treats that as **field-level**
    rather than fatal: `play_action_rate` and `pre_snap_motion_rate` become
    null with a reason and every other rate publishes, because those two are
    the only fields this feed owns.
    """
