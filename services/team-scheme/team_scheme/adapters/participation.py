...
the reason its failure is field-level rather than fatal — losing one field of
thirteen must not cost the other twelve.

nflverse publishes no `.csv.gz` here either; the parquet is 4.52 MiB. See
`docs/collectors.md` for the fleet decision, including the part that matters
most for this feed: 85% of the total available parquet saving is this one
document, buying one field, so de-cadencing it beats adding `pyarrow`.
Measured through the shipped path, projected to 3 of 26 columns: 4.3s for
45,184 rows. The projection matters more here than anywhere else in the fleet
— the columns dropped include six semicolon-joined 22-player lists per row,
which is most of the document's bulk.
...
def classify(offense_personnel: str) -> str | None:
    ...
    *not* what happens — the caller skips it entirely, so an unrecognisable
    ...
    # placed first swallows it and `p13` becomes unreachable. That bug shipped
    # once during the coaching-scheme build and is pinned by the parametrised
    # `classify` table; do not reintroduce it by "simplifying" the order. The
    # spec names p13 explicitly, so it is not a heavy package; `heavy` is the
    # catch-all for the unnamed 4+ groupings (22, 23) and nothing more.
