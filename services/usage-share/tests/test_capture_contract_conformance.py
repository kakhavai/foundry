...
async def test_a_complete_capture_conforms(lake, upstream):
...
async def test_the_published_shares_are_computed_from_the_published_bases(
    lake, upstream
):
    ...
    row = next(r for r in rows if r["player_id"] == canonical_id("00-KC-WR1"))
...
async def test_snaps_and_routes_are_null_rather_than_zero(lake, upstream):
...
async def test_a_full_league_document_reaches_the_declared_floor(lake, serve_upstream):
...
async def test_every_envelope_is_written_to_the_lake(lake, upstream):
    """An envelope that is served but never written leaves no record a week
    later, and the lake is the only durable copy."""
    await capture(lake)
    written = {e.signal_type for e in lake.writes if e.collector == "usage-share"}
    assert written == set(SIGNAL_TYPES)


async def test_a_failed_capture_writes_a_present_zero_envelope(lake, serve_upstream):
    ...
    written = [e for e in lake.writes if e.collector == "usage-share"]
    assert {e.signal_type for e in written} == set(SIGNAL_TYPES)
    assert written, "nothing written — the loop below would pass vacuously"
...
async def test_a_total_outage_reports_a_low_ratio_not_a_perfect_one(
    ...
    written = [e for e in lake.writes if e.collector == "usage-share"]
    assert len(written) == len(SIGNAL_TYPES)
    for envelope in written:
...
async def test_the_sample_document_yields_the_expected_shape(lake, upstream):
