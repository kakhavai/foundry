"""roster-scope's telemetry wiring.
...
the shared module under the env guard, and names itself `roster-scope`.
"""
...
from roster_scope.capture import COLLECTOR_NAME
from roster_scope.main import app


def test_roster_scope_uses_the_shared_telemetry_module():
...
def test_setup_is_called_with_this_collectors_own_name(monkeypatch):
    ...
    assert resolve_service_name(calls[0]) == "roster-scope"
