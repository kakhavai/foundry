"""Fixtures for coaching-scheme's suite.
...
**The four document builders produce the real wire format**, header and all —
...
Every test that goes through `capture_coaching_scheme` therefore exercises the
real streaming parser, the real conditional-GET path, both real guards and the
...
from coaching_scheme.capture import capture_coaching_scheme, reset_published_digests
from coaching_scheme.main import app
...
# --------------------------------------------------------------------------
# games.csv — the real 46-column header
# --------------------------------------------------------------------------
...
_GAMES_COLUMNS = GAMES_HEADER.split(",")
...
def games_row(
    *,
    ...
    values = dict.fromkeys(_GAMES_COLUMNS, "")
    values.update(
    ...
    return ",".join(values[column] for column in _GAMES_COLUMNS)


def games_document(
    coaches: Mapping[str, Mapping[int, str]],
    *,
    season: int = SEASON,
    game_type: str = "REG",
) -> str:
    """`{team: {week: coach}}` -> a games.csv the adapter can really parse.

    Teams are paired off within a week so each row carries two of them, which
    is how the real feed is shaped — one game, two coaches — rather than one
    synthetic row per team.
    """
    rows: list[str] = []
    weeks = sorted({week for by_week in coaches.values() for week in by_week})
    for week in weeks:
        present = [team for team in sorted(coaches) if week in coaches[team]]
        for index in range(0, len(present) - 1, 2):
            away, home = present[index], present[index + 1]
            rows.append(
                games_row(
                    season=season,
                    week=week,
                    away_team=away,
                    home_team=home,
                    away_coach=coaches[away][week],
                    home_coach=coaches[home][week],
                    game_type=game_type,
                )
            )
        if len(present) % 2:
            ...
            rows.append(
                games_row(
                    ...
                    away_team=odd,
                    home_team=present[0],
                    away_coach=coaches[odd][week],
                    home_coach=coaches[present[0]][week],
                    game_type=game_type,
                )
            )
    return "\n".join([GAMES_HEADER, *rows]) + "\n"


def steady_coaches(
    teams: Sequence[str] = TEAMS, *, weeks: int = 12
) -> dict[str, dict[int, str]]:
    """Every team, every week, one coach each. No revision boundaries."""
    return {
        team: {week: f"Coach {team}" for week in range(1, weeks + 1)} for team in teams
    }


def coaches_with_change(
    team: str,
    *,
    at_week: int,
    ...
    """One team's head coach changes at `at_week`; everyone else is steady."""
    grid = steady_coaches(teams, weeks=weeks)
    for week in range(at_week, weeks + 1):
        grid[team][week] = f"Interim {team}"
    return grid
...
def team_week_plays(
    ...
    *,
    ...
    that number — which is what lets the changepoint fixtures state a shift in
    points rather than approximate one.
    """
    if passes is None:
        passes = plays // 2
...
class Feeds:
    ...
    **`etags` makes the mock answer `If-None-Match` the way the real upstream
    ...
    def __init__(
        self,
        *,
        coaches: Mapping[str, Mapping[int, str]] | None = None,
        proe: Mapping[str, Mapping[int, float]] | None = None,
        season: int = SEASON,
        plays_per_week: int = 10,
        games_status: int = 200,
        pbp_status: int = 200,
        ftn_status: int = 200,
        ...
        self.season = season
        self.coaches = coaches if coaches is not None else steady_coaches()
    ...
    def install(self, router) -> None:
        router.get(games_adapter.UPSTREAM_URL).mock(
            side_effect=self._responder(
                "games", games_document(self.coaches, season=self.season).encode()
            )
        )
...
def by_revision(envelopes, signal_type: str) -> dict[str, dict]:
    return {row["revision_id"]: row for row in rows_of(envelopes, signal_type)}
