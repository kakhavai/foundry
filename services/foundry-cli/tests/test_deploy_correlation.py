...
def test_external_dependency_applies_contradiction_penalty():
    near = dict(minutes_before_anomaly=3.0, touched_paths=["services/weather/x.py"], service="weather")
...
def test_deploy_not_touching_service_scores_lower_than_one_that_does():
    base = dict(minutes_before_anomaly=3.0, service="weather", external_dependency_implicated=False)
    touching = score_deploy(touched_paths=["services/weather/main.py"], **base)
    unrelated = score_deploy(touched_paths=["services/player-projections/main.py"], **base)
